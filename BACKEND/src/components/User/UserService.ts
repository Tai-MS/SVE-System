...
import { InferCreationAttributes, UUID } from "sequelize"
...
async function actualizarUsuario(datos: Partial<InferCreationAttributes<Usuario>>, guardarToken: boolean = false): Promise<Usuario | string> {
  ...
  if(guardarToken){
    if(process.env.DEV === "dev"){
      actualizarCampos.token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImRjODZhOTI3LWZkMjQtNDczOC04NjUwLTc0NmQ4M2ExNmUyNiIsImRuaSI6IjQ0MDYyODI4Iiwibm9tYnJlIjoiVEFJRUwiLCJhcGVsbGlkbyI6IlNBR1JFVFRJIiwicm9sIjoiRVNUVURJQU5URSIsImlhdCI6MTc1OTA4NzQ5MSwiZXhwIjoxNzU5MTczODkxfQ.Xm1-upJnFNfyoZGsb17YDzkCgLNu-ZsPofIYz_bfJ_g"
...
//
// meterEnComision  DEBE SER FALSE, ESTA EN TRUE PARA FACILITAR LAS PRUEBAS
//
  ...
  try {
    ...
    await sequelize.transaction(async (t) => {
      for (let alumno of datos) {
        ...
        if(alumno.numero_comision){
          const comision = await Comision.encontrarPorNro(alumno.numero_comision)
